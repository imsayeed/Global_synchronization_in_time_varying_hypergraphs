...
fp1=fopen("omega=0.0_sig1c_sig2c_2.out","w");
...
for(i2=0;i2<=0;i2++)
{
sig1r=0.1;//2.0/100.0*i2;

for(i3=0;i3<=100;i3++)
{
sig1c=-5.0+10.0/100.0*i3;

for(i4=0;i4<=0;i4++)
{
sig2r=0.1;//2.0/100.0*i4;

for(i5=0;i5<=100;i5++)
{
sig2c=-5.0+10.0/100.0*i5;
...
l= omega*ax30-2*sigr*ax20-ev2*(sig1r*ax20-sig1c*ay20+2*alpha2*sqrt(sigr/betar)*((cos(w*t0)*sig2r-sin(w*t0)*sig2c)*ax20+(-cos(w*t0)*sig2c-sin(w*t0)*sig2r)*ay20)); 
k14=l*h;
l=omega*ay30-2*betac*(sigr/betar)*ax20-ev2*(sig1c*ax20+sig1r*ay20+2*alpha2*sqrt(sigr/betar)*((sin(w*t0)*sig2r+cos(w*t0)*sig2c)*ax20+(-sin(w*t0)*sig2c+cos(w*t0)*sig2r)*ay20)); 
 k15=l*h;


l= -omega*ax20-2*sigr*ax30-ev3*(sig1r*ax30-sig1c*ay30+2*alpha2*sqrt(sigr/betar)*((cos(w*t0)*sig2r-sin(w*t0)*sig2c)*ax30+(-cos(w*t0)*sig2c-sin(w*t0)*sig2r)*ay30)); 
k17=l*h;
l=-omega*ay20-2*betac*(sigr/betar)*ax30-ev3*(sig1c*ax30+sig1r*ay30+2*alpha2*sqrt(sigr/betar)*((sin(w*t0)*sig2r+cos(w*t0)*sig2c)*ax30+(-sin(w*t0)*sig2c+cos(w*t0)*sig2r)*ay30)); 
 k18=l*h;
...
l= omega*ax3-2*sigr*ax2-ev2*(sig1r*ax2-sig1c*ay2+2*alpha2*sqrt(sigr/betar)*((cos(w*t)*sig2r-sin(w*t)*sig2c)*ax2+(-cos(w*t)*sig2c-sin(w*t)*sig2r)*ay2)); 
k24=l*h;
l=omega*ay3-2*betac*(sigr/betar)*ax2-ev2*(sig1c*ax2+sig1r*ay2+2*alpha2*sqrt(sigr/betar)*((sin(w*t)*sig2r+cos(w*t)*sig2c)*ax2+(-sin(w*t)*sig2c+cos(w*t)*sig2r)*ay2)); 
 k25=l*h;


l= -omega*ax2-2*sigr*ax3-ev3*(sig1r*ax3-sig1c*ay3+2*alpha2*sqrt(sigr/betar)*((cos(w*t)*sig2r-sin(w*t)*sig2c)*ax3+(-cos(w*t)*sig2c-sin(w*t)*sig2r)*ay3)); 
k27=l*h;
l=-omega*ay2-2*betac*(sigr/betar)*ax3-ev3*(sig1c*ax3+sig1r*ay3+2*alpha2*sqrt(sigr/betar)*((sin(w*t)*sig2r+cos(w*t)*sig2c)*ax3+(-sin(w*t)*sig2c+cos(w*t)*sig2r)*ay3)); 
 k28=l*h;
